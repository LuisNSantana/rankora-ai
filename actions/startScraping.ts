...
const startScraping = async (
  ...
  console.log(`[StartScraping] Job ID: ${jobId}`);
  console.log(`[StartScraping] Webhook URL: ${ENDPOINT}`);
  console.log(`[StartScraping] Encoded webhook: ${encodedEndpoint}`);
  ...
  try {
    ...
    console.log(`[StartScraping] Bright Data response:`, data);

    // Extract snapshot ID from the response and update the job
    if (data && data.snapshot_id) {
      console.log(`[StartScraping] Updating job ${jobId} with snapshot ID: ${data.snapshot_id}`);
      await convex.mutation(api.scrapingJobs.updateJobWithSnapshotId, {
        jobId: jobId as Id<"scrapingJobs">,
        snapshotId: data.snapshot_id,
      });
    } else {
      console.warn(`[StartScraping] No snapshot_id found in response for job ${jobId}`);
    }
